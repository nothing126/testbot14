...
def save_results():
    ...
    with open(os.path.join(results_folder, file_name), 'w', encoding='utf-8') as f:
        for channel in channels:
            if channel_state[channel].get() != "соответствует":
...
for channel in channels:
    ...
    tk.Radiobutton(inner_frame, text="соответствует", variable=channel_state[channel], value="соответствует").grid(
        column=1, row=channels.index(channel))
    tk.Radiobutton(inner_frame, text="нет ЕПГ", variable=channel_state[channel], value="нет ЕПГ").grid(column=2,
                                                                                                       row=channels.index(
                                                                                                           channel))
    tk.Radiobutton(inner_frame, text="черный экран", variable=channel_state[channel], value="черный экран").grid(
        column=3, row=channels.index(channel))
